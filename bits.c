...
int allEvenBits(int x) {
  return !(((x & 0x55) & ((x >> 8) & 0x55) & ((x >> 16) & 0x55) &
            ((x >> 24) & 0x55)) ^
           (0x55));
}
/*
...
int dividePower2(int x, int n) {
  return ((x >> n) + ((!!((x >> n << n) + (~x) + 0x01)) & ((x >> 31) & 0x01)));
}
/*
...
int floatIsEqual(unsigned uf, unsigned ug) {
  if (((uf & 0x7fffffff) == 0x00) && ((ug & 0x7fffffff) == 0x00)) {
    return 1;
  }
  if (((((uf & 0x7fffffff) >> 23) == 0xff) && ((uf & 0x7fffff) != 0x00)) ||
      ((((ug & 0x7fffffff) >> 23) == 0xff) && ((ug & 0x7fffff) != 0x00))) {
    return 0;
  }
  return uf == ug;
}
...
unsigned floatUnsigned2Float(unsigned u) {
  unsigned index, u0, frac, expo, sum, i, last2;
  if (!u)
    return u;
  index = 31;
  u0 = u;
  ...
  frac = ((u >> (index - 0x17)) & 0x007fffff);
  expo = (0x7f + index) << 0x17;
  sum = frac + expo;
  if (index <= 23) {
    return expo + ((u & ((1 << index) - 0x01)) << (23 - index));
  } else {
    last2 = (u >> (index - 24)) & 0x03;
    switch (last2) {
    case 0x01: {
      if (index == 24) {
        return sum;
...
int isAsciiDigit(int x) {
  return ((!((x & (~0x0f)) ^ 0x30)) & ((!(x & 0x08)) | (!((x & 0x0e) ^ 0x08))));
}
/*
...
int logicalNeg(int x) {
  int sign = ((x >> 31) & 0x01);
  return (~sign) & ((~(sign ^ ((((~x) + 0x01) >> 31) & 0x01))) & 0x01);
}
/*
...
unsigned unsignedSatAdd(unsigned x, unsigned y) {
  int MSB = (0x01 << 31);
  int overflow = (!!((((x >> 1) + (y >> 1)) & MSB) |
                     ((x & y & 0x01) & !(~MSB ^ ((x >> 1) + (y >> 1))))));
  overflow = (!overflow) + (~0x00);
  return (overflow + ((x + y) & (~overflow)));
}
/*
 ...
 */
int upperBits(int n) {
  return ((((0x01 << 31) >> ((n + 0x1F) & 0x1F))) & (!n + ~0x00));
}
